...
world.afterEvents.entityDie.subscribe(data => {
  if (!(data.deadEntity.typeId=='minecraft:goat'))return;
    ...
    if (Math.random() >= 0.5) data.deadEntity.dimension.spawnItem(new ItemStack(`poke_horn:fly_goat_horn`,1),data.deadEntity.location);
    else data.deadEntity.dimension.spawnItem(new ItemStack(`poke_horn:resist_goat_horn`,1),data.deadEntity.location);
  }
})
...
world.beforeEvents.worldInitialize.subscribe(data => {
  data.itemComponentRegistry.registerCustomComponent(
    "poke_horn:copper_horn", new PokeCopperHorns()
  )
  data.itemComponentRegistry.registerCustomComponent(
    "poke_horn:goat_horn", new PokeGoatHorn()
  )
})